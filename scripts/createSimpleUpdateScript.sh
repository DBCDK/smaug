...
then
    ...
    echo './createUpdateScript.sh "user:password" "https://auth-admin-stg.dbc.dk" ~/indata/smaug-id-libraries'
    echo './createUpdateScript.sh "user:password" "https://auth-admin.dbc.dk" ~/indata/smaug-id-libraries'
    exit 1
fi
...
do
    ...
    ./updateSmaugClientsWithMoreinfoAccess.js -o "$smaugObject" -p $smaugUserPwd -s $smaugAdminUrl
    echo ""
done
