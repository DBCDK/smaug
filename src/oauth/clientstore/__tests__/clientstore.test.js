...
Object.keys(backends).forEach(backendName => {
  describe(backendName + ' ClientStore', () => {
    ...
    it('should create a new client', function() {
      return clientStore.create(client).then(returnedClient => {
    ...
    it('should retrieve and validate a client', function() {
      const expectedKeys = Object.keys(
        Object.assign({}, client, {id: clientId})
      );
      return clientStore
        .getAndValidate(clientId, clientSecret)
        .should.eventually.include.keys(expectedKeys);
    });

    it('should fail to retrieve a client with the wrong secret', function() {
      return clientStore.getAndValidate(clientId, client.secret + 'foo').should
        .be.rejected;
    });
  });
});
