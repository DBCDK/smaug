...
describe('inmemory ConfigStore', function() {
  ...
  it('should retrieve the default configuration', function() {
    return stores.configStore
      .get(userDecode('anUser@aLibrary'), {id: 'aClient'})
      .should.eventually.deep.equal(config.default);
  });

  it('should retrieve the client-specific configuration', function() {
    return stores.configStore
      .get(userDecode('anUser@aLibrary'), {id: 'appDevLTD'})
      .should.eventually.deep.equal(config.clients.appDevLTD);
  });

  it('should retrieve the library-specific configuration', function() {
    return stores.configStore
      .get(userDecode('anUser@000000'), {id: 'appDevLTD'})
      .should.eventually.deep.equal(config.libraries['000000']);
  });

  it('should retrieve the user-specific configuration', function() {
    return stores.configStore
      .get(userDecode('donald@000000'), {id: 'appDevLTD'})
      .should.eventually.deep.equal(config.users['donald@000000']);
  });
});
